...
const COLLECTION_NAME = 'elementary_service';
...
export async function saveElementaryServiceData(data: any[][]) {
...
export async function getElementaryServiceData() {
  const snapshot = await getDocs(collection(db, COLLECTION_NAME));
  // Return the latest document (by updatedAt)
  let docs: any[] = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  docs = docs.sort((a, b) => (a.updatedAt < b.updatedAt ? 1 : -1));
  return docs[0] || null;
}

export async function updateElementaryServiceData(id: string, data: any[][]) {
  const rowObjects = arrayToRowObjects(data);
