...
import { Plus, Calendar, MapPin, Users, Star, Edit, Trash2, X, Save, Upload, Table, Grid, Clock } from 'lucide-react';
...
const EventsManagement: React.FC = () => {
  ...
  const [formData, setFormData] = useState<EventFormData>({
    ...
    time: '',
    location: '',
    locationEn: '', // keep for type
    category: 'general',
    description: '',
    descriptionEn: '', // keep for type
    capacity: 50, // keep for type
    featured: false, // keep for type
    image: null
  });
  ...
  return (
    <div className="space-y-8 tab-content mt-10">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                            <div className="flex flex-col">
                              ...
                              <div className="flex items-center mt-1">
                                ...
                                {new Date(`1970-01-01T${event.time}`).toLocaleTimeString(language === 'ar' ? 'ar-EG' : 'en-US', { hour: 'numeric', minute: '2-digit', hour12: true })}
                              </div>
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-2 sm:p-4 z-50">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto"  style={{ transform: 'translateX(-5rem)' }}>
    ...
    {/* ConfirmDialog for delete event */}
    <ConfirmDialog
      ...
      title={language === 'ar' ? 'حذف الفعالية' : 'Delete Event'}
      message={
        language === 'ar'
          ? `هل أنت متأكد من حذف الفعالية "${eventToDelete?.title}"؟ لا يمكن التراجع عن هذا الإجراء.`
          : `Are you sure you want to delete the event "${eventToDelete?.titleEn}"? This action cannot be undone.`
      }
